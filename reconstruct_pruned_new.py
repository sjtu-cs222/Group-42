...
if __name__ == "__main__":
	...
	st_retrain = 0
	if keepOn:
		res = os.listdir("./data/exp5_prune")
		for netFile in res:
			last = int(re.sub("\D","",netFile))
			if last > st:
				st = last
		if st > 0:
			net = torch.load("./data/exp5_prune/reconstruct" + str(st) + ".pkl")
	...
	zeros5 = U5
	Z6 = projection(net.convt3.weight.data, configuration.P6)
	U6 = torch.zeros(Z6.size()).cuda()
	zeros6 = U6
	# Record performance
	retrain_loss = []
	test_loss = []
	x_axis = []
	#train and test
	for epoch in range(st, len(learningRate)):
		...
		for i, data in enumerate(trainloader, 0):
			...
			loss = crit(outputs, targets) + 0.00005*(\
				...
				crit(net.convt3.weight.data - Z6 + U6, zeros6))
			accu_loss += loss.item()
			loss.backward()
			optimizer.step()
		...
		Z6 = net.convt3.weight.data + U6
		Z6 = projection(Z6, percent = configuration.P6)
		U6 = U6 + net.convt3.weight.data - Z6
		# Test
		batchNum = 0
		...
		pdf = PdfPages("reconstruct_v4_prune.pdf")
		plt.figure(1)
		...
		pdf.close()
		# Save the net
		net_name = "./data/exp5_prune/reconstruct" + str(epoch+1) + ".pkl"
	...
	for epoch in range(0, 10):
		...
		for i, data in enumerate(trainloader, 0):
			...
			print("net.convt3:")
			net.convt3.weight.data, net.convt3.weight._grad=apply_prune(net.convt3.weight,configuration.P6)
			optimizer.step()
		...
		test_loss.append(accu_loss / batchNum)
		pdf = PdfPages("reconstruct_v4_prune_retrain.pdf")
		plt.figure(1)
		plt.plot(x_axis, retrain_loss, x_axis, test_loss)
		plt.xlabel("epoch")
		plt.ylabel("loss")
		...
		pdf.close()
		# Save the net
		net_name = "./data/exp5_retrain/reconstruct" + str(epoch+1) + ".pkl"
		torch.save(net, net_name)
